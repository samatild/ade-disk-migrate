...
.DESCRIPTION
    ...
    Steps:
      ...
      4. Uploads and runs migrate.sh on the VM via Run Command
...
.EXAMPLE
    ./deploy.ps1 -SubscriptionId "abc-123" -ResourceGroupName "myRG" -VMName "myVM"
    # Replaces source VM in-place with unencrypted OS disk
.EXAMPLE
    ./deploy.ps1 -SubscriptionId "abc-123" -ResourceGroupName "myRG" -VMName "myVM" -Clone
    # Creates a separate clone VM, source is untouched
.EXAMPLE
    ./deploy.ps1   # Prompts for parameters interactively
#>
...
try {
    ...
    if (-not (Test-Path $migrateScript)) {
        Write-Err "migrate.sh not found at: $migrateScript"
        Write-Err "Ensure migrate.sh is in the same directory as deploy.ps1"
        exit 1
    }
    ...
    $uploadCmd = "echo '$b64' | base64 -d > /tmp/migrate.sh && chmod +x /tmp/migrate.sh && echo UPLOAD_OK"

    $upload = Invoke-VMScript -RG $ResourceGroupName -VM $VMName `
        -Script $uploadCmd `
        -Description "Uploading migrate.sh to VM"
    ...
    $runCmd = 'bash /tmp/migrate.sh --yes 2>&1; echo "###MIGRATE_EXIT=$?###"'
